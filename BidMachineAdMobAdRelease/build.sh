...
function rebuild_components {
    ...
    echo -e "${INFO}Build components by scheme BidMachineAdMobAdManager${INFO}"
    ...
    local scheme="BidMachineAdMobAdManager" 
    ...
    cp -r $device_temp_dir/BidMachineAdMobAdManager.framework $universal_temp_dir
...
function merge_components {
    ...
    libtool -static -o "$universal_temp_dir/BidMachineAdMobAdManager.framework/BidMachineAdMobAdManager" "${components_paths[@]}" "${dependencies_paths[@]}"
    mv "$universal_temp_dir/BidMachineAdMobAdManager.framework" "$RELEASE_DIR/BidMachineAdMobAdManager.framework"
}
...
function compress_and_upload {
    ...
    [ "$SHOULD_UPLOAD_TO_S3" = "YES" ] && aws s3 cp "$(PWD)/$BUILD_NAME.zip" "s3://appodeal-ios/BidMachineAdMobAdManager/$VERSION/$BUILD_NAME.zip" --acl public-read
}
...
while test $# -gt 0; do
        case "$1" in
                ...
                -v)
                        ...
                        if test $# -gt 0; then
                                VERSION=$1
                                BUILD_NAME="BidMachineAdMobAdManager-$VERSION" 
                        else
